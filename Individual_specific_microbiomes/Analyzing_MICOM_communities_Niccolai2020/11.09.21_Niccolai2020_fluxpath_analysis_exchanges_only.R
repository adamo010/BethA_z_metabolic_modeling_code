...
sum_analyzed_fluxpaths_filtering_df <- sum_analyzed_fluxpaths_wide_noNANs_zerofree %>%  #create a new dataframe by filtering the old one
  mutate(count_of_nonzeros = rowSums(.[3:4]!=0)) %>% #create a new column, count_of_nonzeroes, that counts # nonzero values in tumor and normal columns
...
sum_analyzed_fluxpaths_filtered <- filter(sum_analyzed_fluxpaths_filter_added, n >= 80)
...
fluxpath_key <- subset(analyzed_fluxpaths, select = c(fluxpath_name, fluxpath_description) )
...
sum_analyzed_fluxpaths_paired_wide <- sum_analyzed_fluxpaths_paired_only %>%
  mutate(Difference_direction = case_when(
    ...
    diff < 0 ~ "tumor < normal",
    diff == 0 ~ "Zero change",))
#I would also like to add back the descriptions, please. (The below two lines should already be done, from above)
...
write.csv(as.data.frame(sum_analyzed_fluxpaths_paried_wide2), file="11.09.21_filtered_sum_exchange_fluxes_only_alldata_Niccolai2020_data.csv")
