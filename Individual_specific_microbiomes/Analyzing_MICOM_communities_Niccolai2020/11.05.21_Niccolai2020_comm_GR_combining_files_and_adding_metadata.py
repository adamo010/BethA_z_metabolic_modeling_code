...
comm_GR_file_list = glob.glob('*_V01_Niccolai_comm_GRs.csv')

comm_GR_file_names = []

for elem in comm_GR_file_list:
    sample_name = re.sub("_V01_Niccolai_comm_GRs.csv", "", elem)  
...
for elem in comm_GR_file_list:
    ...
    sample_name = re.sub("_V01_Niccolai_comm_GRs.csv", "", elem) #delete extra crap on string.
...
os.chdir("/Users/adamo010/Documents/MICOM_CRC_FBA/Individual_specific_microbiomes/Analyzing_MICOM_communities_Niccolai2020/")

with open('Niccolai2020_community_GRs_combined.csv', 'w') as f:
...
TD_point_two = pd.read_csv("Niccolai2020_community_GRs_combined.csv", names= ["Sample_ID", "comm_gr"])
...
comm_GRs_with_metadata = pd.merge(left=TD_point_two, right=metadata, how='left', left_on='Sample_ID', right_on='Sample_ID')
...
comm_GRs_with_metadata.to_csv("11.05.21_Niccolai2020_community_GRs_combined_plus_ranks.csv")
