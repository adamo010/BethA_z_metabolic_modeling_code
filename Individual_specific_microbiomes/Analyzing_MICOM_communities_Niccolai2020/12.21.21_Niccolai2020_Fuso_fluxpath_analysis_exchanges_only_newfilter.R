...
#11.09.21_Hale2018_Fuso_fluxpath_analysis_exchanges_only.R and
#11.10.21_Niccolai2020_Fuso_fluxpath_analysis_exchanges_only.R- here, we're using different (cleaner) filtering
...
filtered_tax <- read_csv("/Users/adamo010/Documents/MICOM_CRC_FBA/Individual_specific_microbiomes/Analyzing_MICOM_communities_Niccolai2020/11.05.21_Niccolai2020_indiv_spp_GRs_collapsed_to_genus.csv")
fuso_only_tax <- filter(filtered_tax, Genus == "Fusobacterium") #filter to only include fuso samples
#why doesn't this dataset...have sample_ids?
fuso_only_tax <- fuso_only_tax %>% 
  mutate(SampleID = paste0(Patient_Blind_ID, "_", Description)) 
...
#clean up
rm(unique_fluxpaths, fluxpaths_unique)
...
num_fuso_pos_samples <- unique(fuso_fluxpaths_collapsed$Sample_ID) #72 samples, here
...
#now, remove all rows where n (the name of the count column) is less than ... 72 is 100%, 36 is 50%
...
write.csv(as.data.frame(fuso_fluxpath_statistics2), file="12.21.21_Fusobacterium_exchange_fluxes_only_stats_Niccolai2020_data.csv")
...
write.csv(as.data.frame(fuso_fluxpaths_paried_wide2), file="12.21.21_Fusobacterium_exchange_fluxes_only_alldata_Niccolai2020_data.csv")

#step 10: graphing!
#Fructose
ggpaired(subset(fuso_fluxpaths_paried_wide2, fluxpath_name %in% c("EX_fru(e)")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Fusobacterium fructose exchange flux,\n Niccolai2020 data"), 
       subtitle = expression("paired Wilcoxan, p=0.00159, q= 0.0697")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
  scale_y_continuous(name = "Sum of Fusobacterium fluxes,\nmmol/(gDW * h)", limits=c(-500,10)) +
...
#Sucrose
ggpaired(subset(fuso_fluxpaths_paried_wide2, fluxpath_name %in% c("EX_sucr(e)")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Fusobacterium sucrose exchange flux,\n Niccolai2020 data"), 
       subtitle = expression("paired Wilcoxan, p=0.00220, q= 0.0697")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
  scale_y_continuous(name = "Sum of Fusobacterium fluxes,\nmmol/(gDW * h)", limits=c(-500,10)) +
  scale_x_discrete(labels = c('Normal','Tumor')) +
  scale_fill_brewer(palette = "Dark2") +
...
ggpaired(subset(fuso_fluxpaths_paried_wide2, fluxpath_name %in% c("EX_val_L(e)")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Fusobacterium L-Valine exchange flux,\n Niccolai2020 data"), 
       subtitle = expression("paired Wilcoxan, p=0.00182, q= 0.0697")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
