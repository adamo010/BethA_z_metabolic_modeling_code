...
#Date_TBA_combining_species_specific_GRs_from_MSI_runs.py
...
filelist = glob.glob('*_V12_Hale_indiv_spp_GRs.csv')

#make a list of sample names
sample_names_list = []
for file in filelist:
    sample_name = re.sub("_V12_Hale_indiv_spp_GRs.csv", "", file) #delete extra crap on string.
...
preprocessed_taxonomy = pd.read_csv("/Users/adamo010/Documents/MICOM_CRC_FBA/Published_data/CRC_OTU_tables_Hale2018/Hale_2018_OTUs_with_matched_taxonomy_and_proxy_OTUids.csv")

dataframe_list = {} #create a dictionary of growth rate files merged with taxonomies

def combining_across_samples(sample_id):
    for file in os.listdir():
        if sample_id in str(file) and str("V12_Hale_indiv_spp_GRs") in str(file):
...
combined_indiv_GRs_with_metadata = pd.merge(left=combined_indiv_GRs, right=metadata, how='left', left_on='Sample_ID', right_on='sampleIDs_from_read_counts')
...
combined_indiv_GRs_with_metadata["unique_id"]= combined_indiv_GRs_with_metadata["OTU_ID"].astype(str) + combined_indiv_GRs_with_metadata["host_subject_id"].astype(str)
...
temp.drop(["OTU_ID", "Species"], axis=1, inplace=True) #not dropping taxonomy_orig, b/c we don't have it. Capitalization also an issue here
...
collapsed_GRs_with_metadata.to_csv("/Users/adamo010/Documents/MICOM_CRC_FBA/Individual_specific_microbiomes/Analyzing_MICOM_communities_Hale2018/09.01.21_Hale2018_indiv_spp_GRs_collapsed_to_genus.csv")
del temp
#collapsing by family
temp2 = copy.deepcopy(combined_indiv_GRs)
temp2.drop(["OTU_ID", "Species", "Genus"], axis=1, inplace=True)
...
collapsed_GRs_with_metadata2.to_csv("/Users/adamo010/Documents/MICOM_CRC_FBA/Individual_specific_microbiomes/Analyzing_MICOM_communities_Hale2018/09.01.21_Hale2018_indiv_spp_GRs_collapsed_to_family.csv")
del temp2

##################step 6: collapsed differences: use combined_indiv_GRs_inner
temp = copy.deepcopy(combined_indiv_GRs_inner)
temp.drop(["OTU_ID", "Species", "cancer_stage", "env_material", "ethnicity", "Host_Age", "host_subject_id", "host_body_mass_index",\
           "host_sex", "host_tissue_sampled", "host_subject_id", "mismatch_repair_status", "collection_site", "biopsy_number",\
...
collapsed_GRs_diffs_with_metadata.to_csv("/Users/adamo010/Documents/MICOM_CRC_FBA/Individual_specific_microbiomes/Analyzing_MICOM_communities_Hale2018/09.01.21_Hale2018_indiv_spp_GRs_collapsed_to_genus_diffs.csv")
