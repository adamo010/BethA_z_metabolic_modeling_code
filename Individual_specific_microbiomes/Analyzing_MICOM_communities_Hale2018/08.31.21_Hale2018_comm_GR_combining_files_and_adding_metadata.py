...
comm_GR_file_list = glob.glob('*_V12_Hale_comm_GRs.csv')

comm_GR_file_names = []

for elem in comm_GR_file_list:
    sample_name = re.sub("_V12_Hale_comm_GRs.csv", "", elem)  
...
for elem in comm_GR_file_list:
    ...
    sample_name = re.sub("_V12_Hale_comm_GRs.csv", "", elem) #delete extra crap on string.
...
with open('Hale2018_V12MSIrun_community_GRs_combined.csv', 'w') as f:
...
TD_point_two = pd.read_csv("Hale2018_V12MSIrun_community_GRs_combined.csv", names= ["Sample_ID", "comm_gr"])
...
comm_GRs_with_metadata = pd.merge(left=TD_point_two, right=metadata, how='left', left_on='Sample_ID', right_on='sampleIDs_from_read_counts')
comm_GRs_with_metadata.drop(columns=['sampleIDs_from_read_counts'], inplace=True)
...
comm_GRs_with_metadata["rank"] = comm_GRs_with_metadata.groupby(["normal_adjacent_or_tumor_tissue_specimen"])["comm_gr"].rank(method='first', ascending=False)
...
comm_GRs_with_metadata.to_csv("08.31.21_Hale2018_V12MSIrun_community_GRs_combined_plus_ranks.csv")
