...
#well, this is smaller, but it's possible some fuso_ids got filtered out. Let's see how far we get with this.
...
num_fuso_pos_samples <- unique(fuso_fluxpaths_collapsed$Sample_ID) #55 samples, here
...
#now, remove all rows where n (the name of the count column) is less than ... 55 is 100%, 44 is 80%, 28 is 50%, 11 is 20%
fuso_fluxpaths_abundance_filtered <- filter(fuso_fluxpaths_filter_added, n >= 55)
...
write.csv(as.data.frame(fuso_fluxpath_statistics2), file="11.09.21_Fusobacterium_exchange_fluxes_only_stats_Hale2018_data.csv")
...
write.csv(as.data.frame(fuso_fluxpaths_paried_wide2), file="11.09.21_Fusobacterium_exchange_fluxes_only_alldata_Hale2018_data.csv")
...
ggpaired(subset(fuso_fluxpaths_paried_wide2, fluxpath_name %in% c("EX_ac(e)")), cond1= "normal", cond2= "tumor",
  ...
  labs(title = expression("Fusobacterium acetate exchange flux,\n Hale2018 data"), subtitle = expression("paired Wilcoxan, p=0.000646, q= 0.0194")) +
...
#L-methionine
ggpaired(subset(fuso_fluxpaths_paried_wide2, fluxpath_name %in% c("EX_met_L(e)")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Fusobacterium L-methionine exchange flux,\n Hale2018 data"), subtitle = expression("paired Wilcoxan, p=0.00162, q= 0.0244")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
  scale_y_continuous(name = "Sum of Fusobacterium fluxes,\nmmol/(gDW * h)", limits=c(-750,0)) +
  scale_x_discrete(labels = c('Normal','Tumor')) +
  scale_fill_brewer(palette = "Dark2") +
  scale_color_brewer(palette = "Dark2") +
  theme(legend.position = "bottom", legend.title=element_blank(), legend.text=element_text(size=11)) +
...
#Proton
ggpaired(subset(fuso_fluxpaths_paried_wide2, fluxpath_name %in% c("EX_h(e)")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Fusobacterium proton exchange flux,\n Hale2018 data"), subtitle = expression("paired Wilcoxan, p=0.00423, q= 0.0423")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
  scale_y_continuous(name = "Sum of Fusobacterium fluxes,\nmmol/(gDW * h)", limits=c(-200,7000)) +
  scale_x_discrete(labels = c('Normal','Tumor')) +
  scale_fill_brewer(palette = "Dark2") +
  scale_color_brewer(palette = "Dark2") +
  theme(legend.position = "bottom", legend.title=element_blank(), legend.text=element_text(size=11)) +
...
#Ammonia
ggpaired(subset(fuso_fluxpaths_paried_wide2, fluxpath_name %in% c("EX_nh4(e)")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Fusobacterium ammonium exchange flux,\n Hale2018 data"), subtitle = expression("paired Wilcoxan, p=0.00620, q= 0.0465")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
  scale_y_continuous(name = "Sum of Fusobacterium fluxes,\nmmol/(gDW * h)", limits=c(-200,5000)) +
  scale_x_discrete(labels = c('Normal','Tumor')) +
  scale_fill_brewer(palette = "Dark2") +
  scale_color_brewer(palette = "Dark2") +
  theme(legend.position = "bottom", legend.title=element_blank(), legend.text=element_text(size=11)) +
