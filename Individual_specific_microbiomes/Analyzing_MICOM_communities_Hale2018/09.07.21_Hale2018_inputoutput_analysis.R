...
analyzed_metabolites <- read_csv("09.02.21_Hale2018_inputoutput_with_metadata_longform.csv")
analyzed_metabolites$host_subject_id <- as.factor(analyzed_metabolites$host_subject_id)
...
analyzed_metabolites_filtered <- filter(analyzed_metabolites_filter_added, n >= 18)
...
#then, do some reordering- maybe this is just a Hale data thing?
analyzed_metabolites_filtered_noNANs <- analyzed_metabolites_filtered_noNANs[, c(1,2,3,5,4,6,7)] #this is edited from previous version, as my output is a bit different
...
analyzed_metabolites_paired <- analyzed_metabolites_filtered_noNANs %>% unite("sorting_col", metabolite_name:host_subject_id, remove = FALSE)
...
#after all this filtering, we get 1072 metabolite-Patient_blind_id combinations. 
...
by_metab_stats <- lapply(split_analyzed_metabolites_by_metab, function(df){wilcox.test(metabolite_amount~normal_adjacent_or_tumor_tissue_specimen, data=df, exact= FALSE, paired= TRUE)})
...
write.csv(as.data.frame(metabolite_statistics), file="09.07.21_filtered_microbial_input_output_sig_results_Hale2018_data.csv")
...
analyzed_metabolites_paired_wide = subset(analyzed_metabolites_paired_only, select = -c(...1,n,SampleID) )
#Split up to wide.
analyzed_metabolites_paired_wide <- spread(analyzed_metabolites_paired_wide, normal_adjacent_or_tumor_tissue_specimen, metabolite_amount)
...
write.csv(as.data.frame(analyzed_metabolites_paired_wide), file="09.07.21_filtered_microbial_input_output_data_Hale2018_data.csv")

#step 10: NOW GRAPH??!!!
#Biotin
ggpaired(subset(analyzed_metabolites_paired_wide, metabolite_name %in% c("Biotin")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Biotin export, Hale2018 data"), subtitle = expression("paired Wilcoxan, p=0.0826, q= 0.910")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5)) +
  scale_y_continuous(name = "Average flux,\nmmol/(gDW * h)", limits=c(0,0.008)) +
...
ggpaired(subset(analyzed_metabolites_paired_wide, metabolite_name %in% c("released mucin-type O-glycan No 9")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Released mucin-type O-glycan No 9, Hale2018 data"), subtitle = expression("paired Wilcoxan, p=0.0828, q= 0.910")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5)) +
  scale_y_continuous(name = "Average flux,\nmmol/(gDW * h)", limits=c(0,0.6)) +
  scale_x_discrete(labels = c('Normal','Tumor')) +
  scale_fill_brewer(palette = "Dark2") +
  scale_color_brewer(palette = "Dark2") +
  theme(legend.position = "bottom", legend.title=element_blank(), legend.text=element_text(size=11)) +
...
#all right, just save data and move on.
