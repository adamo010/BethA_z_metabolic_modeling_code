...
#it seems like there are 18 samples which have zero values. These are zeroes for heme and seroheme.So... let's just ignore it.
...
analyzed_metabolites_wide <- select(analyzed_metabolites_collapsed, -c(...1, SampleID)) #drop unnecessary columns
#first, convert to wide form
analyzed_metabolites_wide <- analyzed_metabolites_wide %>% 
  spread(normal_adjacent_or_tumor_tissue_specimen, metabolite_amount)
...
analyzed_metabolites_paired_long <- reshape2::melt(data= analyzed_metabolites_paired_only,
                                              id.vars= c("host_subject_id", "metabolite_name"),
                                              variable.name = "Description",
                                              value.name = "metabolite_amount")
#split up by different metabolitesand see where we get. 
...
write.csv(as.data.frame(metabolite_statistics), file="09.24.21_filtered_microbial_input_output_all_results_Hale2018_data.csv")
...
write.csv(as.data.frame(metabolite_statistics_sig), file="09.24.21_filtered_microbial_input_output_SIG_results_Hale2018_data.csv")


##########Step 9: prepare data for graphing NOT DONE IN THIS VERSION
...
ggpaired(subset(analyzed_metabolites_paired_wide, metabolite_name %in% c("Adenosine monophosphate")), cond1= "normal", cond2= "tumor",
  ...
  labs(title = expression("Adenosine monophosphate export, Hale2018 data"), subtitle = expression("paired Wilcoxan, p=0.0446, q= 0.579")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5)) +
  scale_y_continuous(name = "Average flux,\nmmol/(gDW * h)", limits=c(0,0.2)) +
