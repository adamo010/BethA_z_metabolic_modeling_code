...
def main():
    ...
    model_only_MM =  minimal_medium(imported_pickle_model, 1.0, minimize_components=True) #old way of getting model_only_MM; go back to this in V16.
    ...
    del key, value
    ###Step 4 NEW: straight up merge these dicts. Keep highest value for each key. 
    ...
    #sample_driven_diet_amounts = dict(**to_keep_from_EU_diet, **model_only_MM_dict)
    ...
    post_coop_med = minimal_medium(imported_pickle_model, sample_sol.growth_rate, min_growth=rates, exports=True, minimize_components=True) 
    ...
    if post_coop_med is None:
    	...
    	with open("_V5_Hale_input_output.csv", "w") as text_file:
    		text_file.write(output)    
    else:
    	post_coop_med.to_csv("_V5_Hale_input_output.csv")
    #with open("_V3_Hale_comm_GRs.csv", "w") as text_file:
        #text_file.write(comm_GR)
    for file in os.listdir():
        src=file
        if fnmatch.fnmatch(file, "_V5_Hale_input_output.csv"):
