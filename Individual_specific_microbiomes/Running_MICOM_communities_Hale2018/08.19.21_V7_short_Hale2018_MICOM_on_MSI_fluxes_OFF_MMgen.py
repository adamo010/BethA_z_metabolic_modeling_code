...
def main():
    ...
    sample_sol = imported_pickle_model.cooperative_tradeoff(fraction=0.20) 
    ...
    if post_coop_med is None:
    	...
    	with open("_V7_Hale_input_output.csv", "w") as text_file:
    		text_file.write(output)    
    else:
    	post_coop_med.to_csv("_V7_Hale_input_output.csv")
    #with open("_V3_Hale_comm_GRs.csv", "w") as text_file:
        #text_file.write(comm_GR)
    for file in os.listdir():
        src=file
        if fnmatch.fnmatch(file, "_V7_Hale_input_output.csv"):
