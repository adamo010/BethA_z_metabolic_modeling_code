...
def main():
    ...
    sample_sol = imported_pickle_model.cooperative_tradeoff(fraction=0.20, fluxes=True, pfba=True) 
    ...
    with open('_V7_Hale_MSIgen_MM_from_model.csv', 'w') as f:
        for key in model_only_MM_dict.keys():
            f.write("%s,%s\n"%(key,model_only_MM_dict[key]))
    del f, key
    rates.to_csv("_V7_Hale_indiv_spp_GRs.csv")
    fluxes.to_csv("_V7_Hale_fluxes.csv")
    ...
    with open("_V7_Hale_comm_GRs.csv", "w") as text_file:
    ...
    for file in os.listdir():
        src=file
        	...
        	#os.rename(src,dst)   
        if fnmatch.fnmatch(file, "_V7_Hale_MSIgen_MM_from_model.csv"):
            dst=str(pickle_file_sample_name)+file
            os.rename(src,dst)
        elif fnmatch.fnmatch(file, "_V7_Hale_indiv_spp_GRs.csv"):
            dst=str(pickle_file_sample_name)+file
            os.rename(src,dst) 
        elif fnmatch.fnmatch(file, "_V7_Hale_fluxes.csv"):
            dst=str(pickle_file_sample_name)+file
            os.rename(src,dst)    
        elif fnmatch.fnmatch(file, "_V7_Hale_comm_GRs.csv"):
        	dst=str(pickle_file_sample_name)+file
        	os.rename(src,dst)       			
    return
