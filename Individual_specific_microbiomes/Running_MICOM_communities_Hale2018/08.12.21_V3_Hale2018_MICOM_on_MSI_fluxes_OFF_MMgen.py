...
def main():
    ...
    sample_sol = imported_pickle_model.cooperative_tradeoff(min_growth=0, fraction=0.33, fluxes=False, pfba=True) 
    ...
    if post_coop_med is not None:
    	post_coop_med.to_csv("_V3_Hale_input_output.csv")
    #with open("_V3_Hale_comm_GRs.csv", "w") as text_file:
        #text_file.write(comm_GR)
    for file in os.listdir():
        src=file
        if fnmatch.fnmatch(file, "_V3_Hale_input_output.csv"):
