...
def main():
    ...
    with open('_V10_Hale_MSIgen_MM_from_model.csv', 'w') as f:
        for key in model_only_MM_dict.keys():
            f.write("%s,%s\n"%(key,model_only_MM_dict[key]))
    del f, key
    with open('_V10_Hale_MSIgen_model_plus_diet_MM.csv', 'w') as f:
    ...
    post_coop_med = minimal_medium(imported_pickle_model, sample_sol.growth_rate*0.95, min_growth=rates*0.95, exports=True) 
    comm_GR = str(sample_sol)
    os.chdir("/panfs/roc/groups/7/blekhman/adamo010/Combining_Hale_models/Hale_MICOM_output/")
    rates.to_csv("_V10_Hale_indiv_spp_GRs.csv")
    fluxes.to_csv("_V10_Hale_fluxes.csv")
    if post_coop_med is None:
    	output = "This sample has no post coop med"
    	with open("_V10_Hale_input_output.csv", "w") as text_file:
    		text_file.write(output)    
    else:
    	post_coop_med.to_csv("_V10_Hale_input_output.csv")
    with open("_V10_Hale_comm_GRs.csv", "w") as text_file:
        text_file.write(comm_GR)
    #now, rename all files
    for file in os.listdir():
        src=file
        if fnmatch.fnmatch(file, "_V10_Hale_MSIgen_MM_from_model.csv"):
            dst=str(pickle_file_sample_name)+file
            os.rename(src,dst)
        elif fnmatch.fnmatch(file, "_V10_Hale_MSIgen_model_plus_diet_MM.csv"):
        	dst=str(pickle_file_sample_name)+file
        	os.rename(src,dst)
        elif fnmatch.fnmatch(file, "_V10_Hale_indiv_spp_GRs.csv"):
            dst=str(pickle_file_sample_name)+file
            os.rename(src,dst) 
        elif fnmatch.fnmatch(file, "_V10_Hale_fluxes.csv"):
            dst=str(pickle_file_sample_name)+file
            os.rename(src,dst)    
        elif fnmatch.fnmatch(file, "_V10_Hale_input_output.csv"):
        	dst=str(pickle_file_sample_name)+file
        	os.rename(src,dst)
        elif fnmatch.fnmatch(file, "_V10_Hale_comm_GRs.csv"):
        	dst=str(pickle_file_sample_name)+file
        	os.rename(src,dst)	   	            	       			
    return
