...
def main():
    ...
    sample_sol = imported_pickle_model.cooperative_tradeoff(min_growth=0, fraction=0.33, fluxes=True, pfba=True) 
    ...
    #save model only MM
    with open('_V1_Hale_MSIgen_MM_from_model.csv', 'w') as f:
        for key in model_only_MM_dict.keys():
            f.write("%s,%s\n"%(key,model_only_MM_dict[key]))
    del f, key
    rates.to_csv("_V1_Hale_indiv_spp_GRs.csv")
    fluxes.to_csv("_V1_Hale_fluxes.csv")
    if post_coop_med is not None:
    	post_coop_med.to_csv("_V1_Hale_input_output.csv")
    with open("_V1_Hale_comm_GRs.csv", "w") as text_file:
        text_file.write(comm_GR)
    for file in os.listdir():
        src=file
        if fnmatch.fnmatch(file, "_V1_Hale_MSIgen_MM_from_model.csv"):
            dst=str(pickle_file_sample_name)+file
            os.rename(src,dst)
        elif fnmatch.fnmatch(file, "_V1_Hale_indiv_spp_GRs.csv"):
            dst=str(pickle_file_sample_name)+file
            os.rename(src,dst) 
        elif fnmatch.fnmatch(file, "_V1_Hale_fluxes.csv"):
            dst=str(pickle_file_sample_name)+file
            os.rename(src,dst)
        elif os.path.isfile("_V1_Hale_input_output.csv") and fnmatch.fnmatch(file, "_V1_Hale_input_output.csv"):
        	dst=str(pickle_file_sample_name)+file
        	os.rename(src,dst)    
        elif fnmatch.fnmatch(file, "_V1_Hale_comm_GRs.csv"):
        	dst=str(pickle_file_sample_name)+file
        	os.rename(src,dst)	
    return
