...
filelist = glob.glob('*_V01_Muehlbauer_fluxes.csv') #why haven't I been doing this forever? this creates a list of files with the specified string

sample_names_list = []
for file in filelist:
    sample_name = re.sub("_V01_Muehlbauer_fluxes.csv", "", file) #delete extra crap on string.  
...
#INTERESTING:  192 fluxpaths for Burns, 324 for Hale, 217 for Niccolai, 160 for Muehlauber
...
combo_fluxpath_df_plus_metab_sample_metadata = pd.merge(left=combo_fluxpath_plus_metab_metadata, right=metadata, 
                                                        how='left', left_on='Sample_ID', right_on='Sample_name') #merge metadata
combo_fluxpath_df_plus_metab_sample_metadata.drop(['Sample_name'], axis=1, inplace=True)#drop superfluous columns

#export!
os.chdir("/Users/adamo010/Documents/MICOM_CRC_FBA/Individual_specific_microbiomes/Analyzing_MICOM_communities_Muehlbauer2021/") #analysis directory
combo_fluxpath_df_plus_metab_sample_metadata.to_csv("11.09.21_Muehlbauer2021_fluxes_with_metadata_longform.csv")
