...
#and 09.03.21_Hale2018_comm_GR_analysis.R
...
group_GR_all <- read_csv("11.05.21_Niccolai2020_community_GRs_combined_plus_ranks.csv")
...
group_GR_all <- select(group_GR_all, -c(...1, Sample_ID, rank)) #drop the columns called ...1 (the index column, and the sample ID column)
...
group_GR_all_wide <- group_GR_all_wide %>%
  mutate(mean_GR_difference=tumor-normal)
...
#GRAPH!!!
ggpaired(group_GR_all_wide, cond1= "normal", cond2= "tumor",
         y= "growth_rate", fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Community growth rates, Niccolai2020 data"), subtitle = expression("paired Wilcoxan, p=0.0625")) +
