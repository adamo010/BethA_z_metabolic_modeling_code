...
#based on 12.28.21_Muehlbauer2021_indiv_spp_GRs_combining_files_and_adding_metadata.py
...
#we are NOT doing that here. Just keep everything consistent. 
...
temp = copy.deepcopy(combined_indiv_GRs_with_metadata)
temp.columns.values
temp.drop(["phylum_level_sort", "class_level_sort", "order_level_sort", "family_level_sort", "genus_level_sort", "species_level_sort",
           "taxonomy_orig", "Library_name", "Isolate", "experimental_treatment"], axis=1, inplace=True)
collapsed_GRs = temp.groupby(["Sample_ID", 'kingdom', 'phylum', 'class', 'order', 'family', 'genus']).mean()
...
collapsed_GRs_with_metadata = pd.merge(left=collapsed_GRs, right=metadata, how='left', left_on='Sample_ID', right_on='Sample_name')
collapsed_GRs_with_metadata.drop(["Sample_name"], axis=1, inplace=True)
collapsed_GRs_with_metadata.to_csv("/Users/adamo010/Documents/MICOM_CRC_FBA/Individual_specific_microbiomes/Analyzing_MICOM_communities_Muehlbauer2021/05.02.22_Muehlbauer2021_indiv_spp_GRs_collapsed_to_genus.csv")
