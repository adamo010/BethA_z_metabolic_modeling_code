...
#and 09.22.21_correlating_Burns_GEs_and_change_in_inputoutputs_trace_minerals_collapsed.R
...
############step 1: import files
top_metabs <- read_csv("09.24.21_filtered_microbial_input_output_all_results_Burns2015_data_trace_elements_combined.csv")
metabs_all <-read_csv("09.24.21_filtered_microbial_input_output_data_Burns2015_data_trace_elements_combined.csv")

#goal: filter this dataframe by the top_metabs dataframe. In that, metabolite_amount has the identifier metabolite_name

############step 2: filter metabs_all 
####now, the filtering step. here we decide how many correlations we want to do.
#have two options: use all (7), or use only those with pvals > 0.1 (2). Create vectors for each
all_metabs_names <- pull(top_metabs,metab_ids) #make a vector from a column:thanks dplyr!
pval_metabs_names <- all_metabs_names[1:2] #select top two elements from the vector

#use that to filter on the new column
metabs_all_diffs <- filter(metabs_all, metabolite_name %in% all_metabs_names) #thanks again dplyr!
metabs_pval_diffs <- filter(metabs_all, metabolite_name %in% pval_metabs_names) #thanks again dplyr!
...
metabs_all_diffs <- metabs_all_diffs %>% mutate(testo = normal * tumor)
metabs_pval_diffs <- metabs_pval_diffs %>% mutate(testo = normal * tumor)
#we're good. delete that sorting col.
metabs_all_diffs <- metabs_all_diffs[ , ! names(metabs_all_diffs) %in% c("testo")]
metabs_pval_diffs <- metabs_pval_diffs[ , ! names(metabs_pval_diffs) %in% c("testo")]
...
#ONLY need to do this for metabs_all_diffs: metabs_pval_diffs does not have any exports.
imported_metabs_all_diffs <- metabs_all_diffs[which(metabs_all_diffs$tumor > 0),]
exported_metabs_all_diffs <- metabs_all_diffs[which(metabs_all_diffs$tumor < 0),]
#for exported metabolites, need to convert to +ve values.
exported_metabs_all_diffs <- exported_metabs_all_diffs %>% mutate(tumor= abs(tumor), normal=abs(normal)) #absolute value
...
exported_metabs_all_diffs$normal <- exported_metabs_all_diffs$normal + 1
exported_metabs_all_diffs$tumor <- exported_metabs_all_diffs$tumor + 1
#
imported_metabs_all_diffs$normal <- imported_metabs_all_diffs$normal + 1
imported_metabs_all_diffs$tumor <- imported_metabs_all_diffs$tumor + 1
#
metabs_pval_diffs$normal <- metabs_pval_diffs$normal + 1
metabs_pval_diffs$tumor <- metabs_pval_diffs$tumor + 1

#calculate log2 values and put into new column
exported_metabs_all_diffs[,c("log2_metabs_tumor")] <- log2(exported_metabs_all_diffs[,c("tumor")])
exported_metabs_all_diffs[,c("log2_metabs_normal")] <- log2(exported_metabs_all_diffs[,c("normal")])
###
imported_metabs_all_diffs[,c("log2_metabs_tumor")] <- log2(imported_metabs_all_diffs[,c("tumor")])
imported_metabs_all_diffs[,c("log2_metabs_normal")] <- log2(imported_metabs_all_diffs[,c("normal")])
###
metabs_pval_diffs[,c("log2_metabs_tumor")] <- log2(metabs_pval_diffs[,c("tumor")])
metabs_pval_diffs[,c("log2_metabs_normal")] <- log2(metabs_pval_diffs[,c("normal")])

#NEW here: instead of log2TumorValues, get the difference: this gives log2fold change.
exported_metabs_all_diffs$log2_metabs_diffs <- (exported_metabs_all_diffs$log2_metabs_tumor - exported_metabs_all_diffs$log2_metabs_normal)
imported_metabs_all_diffs$log2_metabs_diffs <- (imported_metabs_all_diffs$log2_metabs_tumor - imported_metabs_all_diffs$log2_metabs_normal)
metabs_pval_diffs$log2_metabs_diffs <- (metabs_pval_diffs$log2_metabs_tumor - metabs_pval_diffs$log2_metabs_normal)

############Step 6: clean up a couple of columns
exported_metabs_all_diffs_cleaned <- subset(exported_metabs_all_diffs, select = -c(...1, diff, Difference_direction, 
                                                                                   normal, tumor, log2_metabs_normal, log2_metabs_tumor) )
imported_metabs_all_diffs_cleaned <- subset(imported_metabs_all_diffs, select = -c(...1, diff, Difference_direction, 
                                                                                   normal, tumor, log2_metabs_normal, log2_metabs_tumor) )
metabs_pval_diffs_cleaned <- subset(metabs_pval_diffs, select = -c(...1, diff, Difference_direction, 
                                                                                   normal, tumor, log2_metabs_normal, log2_metabs_tumor) )

#cleanup
rm(exported_metabs_all_diffs, imported_metabs_all_diffs, metabs_pval_diffs)
...
exported_metabs_all_diffs_cleaned_wide <- subset(exported_metabs_all_diffs_cleaned, select = -c(metabolite_name) )
exported_metabs_all_diffs_cleaned_wide <- rename(exported_metabs_all_diffs_cleaned_wide, "D-Galactose" = "log2_metabs_diffs")
#sort by Patient_Blind_ID, then set Patient_Blind_ID as rownames
exported_metabs_all_diffs_cleaned_wide <- exported_metabs_all_diffs_cleaned_wide[order(exported_metabs_all_diffs_cleaned_wide$Patient_Blind_ID),]
exported_metabs_all_diffs_cleaned_wide <- column_to_rownames(exported_metabs_all_diffs_cleaned_wide, var = "Patient_Blind_ID") #set Patient_Blind_ID as row names

#these ones work fine.
imported_metabs_all_diffs_cleaned_wide <- spread(imported_metabs_all_diffs_cleaned, metabolite_name, log2_metabs_diffs) 
metabs_pval_diffs_cleaned_wide <- spread(metabs_pval_diffs_cleaned, metabolite_name, log2_metabs_diffs) 

#sort by Patient_Blind_ID, then set Patient_Blind_ID as rownames
imported_metabs_all_diffs_cleaned_wide <- imported_metabs_all_diffs_cleaned_wide[order(imported_metabs_all_diffs_cleaned_wide$Patient_Blind_ID),]
imported_metabs_all_diffs_cleaned_wide <- column_to_rownames(imported_metabs_all_diffs_cleaned_wide, var = "Patient_Blind_ID") #set Patient_Blind_ID as row names
#exported_metabs_all_diffs_cleaned_wide has already been sorted and set
metabs_pval_diffs_cleaned_wide <- metabs_pval_diffs_cleaned_wide[order(metabs_pval_diffs_cleaned_wide$Patient_Blind_ID),]
metabs_pval_diffs_cleaned_wide <- column_to_rownames(metabs_pval_diffs_cleaned_wide, var = "Patient_Blind_ID") #set Patient_Blind_ID as row names
...
#first, get the ids
export_row_names <- row.names(exported_metabs_all_diffs_cleaned_wide) 
...
#to correlate: exported_metabs_all_diffs_cleaned_wide, imported_metabs_all_diffs_cleaned_wide, metabs_pval_diffs_cleaned_wide
#(import_correlations_uncorr_pvals --> import_corrs_all_metabs; export_correlations_uncorr_pvals --> export_corrs_all_metabs;)
#NOTE!!!! this single command generates both raw and adjusted p-values- call p for unadjusted, and p.adj for adjusted.
import_corrs_all_metabs <- corr.test(imported_metabs_all_diffs_cleaned_wide, host_gene_expression,
                                    use = "pairwise", method="spearman", adjust="BH", alpha=.05, ci=TRUE, minlength=5)
export_corrs_all_metabs <- corr.test(exported_metabs_all_diffs_cleaned_wide, host_gene_expression_trunc,
                                     use = "pairwise", method="spearman", adjust="BH", alpha=.05, ci=TRUE, minlength=5)
pval_corrs_all_metabs <- corr.test(metabs_pval_diffs_cleaned_wide, host_gene_expression,
                                     use = "pairwise", method="spearman", adjust="BH", alpha=.05, ci=TRUE, minlength=5)
#eggcellent. make some dataframes
###imports_all
imports_all_corr_p_values_corrected <- as.data.frame(import_corrs_all_metabs$p.adj) #this makes a dataframe
imports_all_corr_p_values_corrected <- tibble::rownames_to_column(imports_all_corr_p_values_corrected, "metab_id") #THANKS DPLYR, hero of R!
imports_all_corr_p_values_uncorrected <- as.data.frame(import_corrs_all_metabs$p) #this makes a dataframe
imports_all_corr_p_values_uncorrected <- tibble::rownames_to_column(imports_all_corr_p_values_uncorrected, "metab_id") #THANKS DPLYR, hero of R!
###exports_all
exports_all_corr_p_values_corrected <- as.data.frame(export_corrs_all_metabs$p.adj) #this makes a dataframe
exports_all_corr_p_values_corrected <- tibble::rownames_to_column(exports_all_corr_p_values_corrected, "metab_id") #THANKS DPLYR, hero of R!
exports_all_corr_p_values_uncorrected <- as.data.frame(export_corrs_all_metabs$p) #this makes a dataframe
exports_all_corr_p_values_uncorrected <- tibble::rownames_to_column(exports_all_corr_p_values_uncorrected, "metab_id") #THANKS DPLYR, hero of R!
###imports and exports for pval>0.1 metabolites
metabs_sigpvals_corr_p_values_corrected <- as.data.frame(pval_corrs_all_metabs$p.adj) #this makes a dataframe
metabs_sigpvals_corr_p_values_corrected <- tibble::rownames_to_column(metabs_sigpvals_corr_p_values_corrected, "metab_id") #THANKS DPLYR, hero of R!
metabs_sigpvals_corr_p_values_uncorrected <- as.data.frame(pval_corrs_all_metabs$p) #this makes a dataframe
metabs_sigpvals_corr_p_values_uncorrected <- tibble::rownames_to_column(metabs_sigpvals_corr_p_values_uncorrected, "metab_id") #THANKS DPLYR, hero of R!
...
imports_all_corr_p_values_corrected_long <- imports_all_corr_p_values_corrected %>%
  melt(id.var="metab_id") %>%
  arrange(metab_id, variable) 
imports_all_corr_p_values_corrected_long <- rename(imports_all_corr_p_values_corrected_long, c("gene_id"="variable", "p_value_adj"="value")) #rename columns
#
imports_all_corr_p_values_uncorrected_long <- imports_all_corr_p_values_uncorrected %>%
  melt(id.var="metab_id") %>%
  arrange(metab_id, variable)
imports_all_corr_p_values_uncorrected_long <- rename(imports_all_corr_p_values_uncorrected_long, c("gene_id"="variable", "p_value_raw"="value")) #rename columns
#ooo, can we merge these? 
imports_all_corr_p_values_long_merged <- merge(imports_all_corr_p_values_corrected_long,
                                                           imports_all_corr_p_values_uncorrected_long, by=c("metab_id","gene_id"))
imports_all_corr_p_values_long_merged <- imports_all_corr_p_values_long_merged[order(imports_all_corr_p_values_long_merged$p_value_raw),] #sort by p-value
###
exports_all_corr_p_values_corrected_long <- exports_all_corr_p_values_corrected %>%
  melt(id.var="metab_id") %>%
  arrange(metab_id, variable) 
exports_all_corr_p_values_corrected_long <- rename(exports_all_corr_p_values_corrected_long, c("gene_id"="variable", "p_value_adj"="value")) #rename columns
#
exports_all_corr_p_values_uncorrected_long <- exports_all_corr_p_values_uncorrected %>%
  melt(id.var="metab_id") %>%
  arrange(metab_id, variable)
exports_all_corr_p_values_uncorrected_long <- rename(exports_all_corr_p_values_uncorrected_long, c("gene_id"="variable", "p_value_raw"="value")) #rename columns
#ooo, can we merge these? 
exports_all_corr_p_values_long_merged <- merge(exports_all_corr_p_values_corrected_long,
                                               exports_all_corr_p_values_uncorrected_long, by=c("metab_id","gene_id"))
exports_all_corr_p_values_long_merged <- exports_all_corr_p_values_long_merged[order(exports_all_corr_p_values_long_merged$p_value_raw),] #sort by p-value
###
metabs_sigpvals_corr_p_values_corrected_long <- metabs_sigpvals_corr_p_values_corrected %>%
  melt(id.var="metab_id") %>%
  arrange(metab_id, variable) 
metabs_sigpvals_corr_p_values_corrected_long <- rename(metabs_sigpvals_corr_p_values_corrected_long, c("gene_id"="variable", "p_value_adj"="value")) #rename columns
#
metabs_sigpvals_corr_p_values_uncorrected_long <- metabs_sigpvals_corr_p_values_uncorrected %>%
  melt(id.var="metab_id") %>%
  arrange(metab_id, variable)
metabs_sigpvals_corr_p_values_uncorrected_long <- rename(metabs_sigpvals_corr_p_values_uncorrected_long, c("gene_id"="variable", "p_value_raw"="value")) #rename columns
#ooo, can we merge these? 
metabs_sigpvals_corr_p_values_long_merged <- merge(metabs_sigpvals_corr_p_values_corrected_long,
                                               metabs_sigpvals_corr_p_values_uncorrected_long, by=c("metab_id","gene_id"))
metabs_sigpvals_corr_p_values_long_merged <- metabs_sigpvals_corr_p_values_long_merged[order(metabs_sigpvals_corr_p_values_long_merged$p_value_raw),] #sort by p-value


#save THESE to excel
list_of_datasets <- list("all_metabs_imports_GE_corrs" = imports_all_corr_p_values_long_merged,
                         "all_metabs_exports_GE_corrs" = exports_all_corr_p_values_long_merged,
                         "psig_metabs_GE_corrs" = metabs_sigpvals_corr_p_values_long_merged)
write.xlsx(list_of_datasets, "09.28.21_Burns2015_metabolic_gene_expression_microbe_import_export_correlations.xlsx", row.names= FALSE, col.names=TRUE)
