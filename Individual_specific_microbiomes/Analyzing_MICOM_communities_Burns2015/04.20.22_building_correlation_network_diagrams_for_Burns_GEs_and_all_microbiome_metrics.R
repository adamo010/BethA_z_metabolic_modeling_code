...
#hmmm.... I'm not sure this is exactly the right data structure... well, we'll try it

###########Make a network
#don't need to fortify, because we're already working with a dataframe
...
#then, create network
#gr_ge_corrs.net <- network::network(gr_ge_corrs[, 1:2], directed = TRUE) #set directed to TURE or will get duplicate edges error

###########Create some network features
# create edge attribute (R_value)
#network::set.edge.attribute(gr_ge_corrs.net, "R_value", #SOMETHING GOES HERE FUCK IF I KNOW WHAT IT IS)
#network::get.edge.attribute(gr_ge_corrs.net, "R_value")
#network::set.edge.attribute(gr_ge_corrs.net, "R_value", gr_ge_corrs$R_value[, 2 ]) #5th column is R_value

#I want a node feature where we color host and microbiome nodes differently
#take a look at our network file structure:
head(ggnetwork(gr_ge_corrs))
tail(ggnetwork(gr_ge_corrs))
#network::set.vertex.attribute(gr_ge_corrs, "org_type", c(rep("event", 14), rep("woman", 18)))

###########Graph
#see this site for layout.alg options: https://www.rdocumentation.org/packages/sna/versions/2.5/topics/gplot.layout
ggplot(data= ggnetwork(gr_ge_corrs, layout.alg = "circle"),
       aes(x, y, xend=xend, yend=yend)) +
  geom_edges(aes(size = R_value)) + #sets edge widths to n
  geom_nodes(size = 4) + 
  theme_blank() +
  theme(legend.position = "bottom", legend.box = "horizontal", plot.margin = margin(0,0,0,0, "cm")) +
  geom_nodelabel(aes(label = vertex.names)) 
  
#next steps: make genes and taxa different colors; make positive and negative interactions different colors (will have to go back
#to actual correlation data)
...
#let's graph again
library(wesanderson)
ggplot(data= ggnetwork(gr_ge.net, layout.alg = "eigen"), 
       aes(x, y, xend=xend, yend=yend)) +
  geom_edges(aes(size = R_value)) + #sets edge widths to n
  geom_nodes(aes(colour=factor(node_type)), size = 4) + 
  #geom_nodelabel(aes(label=vertex.names, colour=factor(node_type), size=1.5)) +
  geom_nodelabel_repel(aes(label=vertex.names, colour=factor(node_type), size=2), 
                       force = 2, label.padding=unit(0.15, "cm"), point.padding = unit(0.15, "cm")) +
  theme_blank() +
  #theme(legend.position = "bottom", legend.box = "horizontal", plot.margin = margin(0,0,0,0, "cm")) +
  scale_colour_manual("Node_type", labels = c("Taxon growth rate","Host gene"),
...
#let's graph again
library(wesanderson)
ggplot(data= ggnetwork(flux_ge.net, layout.alg = "none"), 
       aes(x, y, xend=xend, yend=yend)) +
  geom_edges(aes(size = R_value)) + #sets edge widths to n
  geom_nodes(aes(colour=factor(node_type)), size = 4) + 
  #geom_nodelabel(aes(label=vertex.names, colour=factor(node_type), size=1.5)) +
...
plot1 <- ggplot(data= ggnetwork(gr_ge.net, layout.alg = "eigen"), 
       aes(x, y, xend=xend, yend=yend)) +
...
plot2 <- ggplot(data= ggnetwork(flux_ge.net, layout.alg = "eigen"), 
       aes(x, y, xend=xend, yend=yend)) +
  geom_edges(aes(size = R_value, color= corr_dir)) + #sets edge widths to n
  geom_nodes(aes(colour=factor(node_type)), size = 5) + 
