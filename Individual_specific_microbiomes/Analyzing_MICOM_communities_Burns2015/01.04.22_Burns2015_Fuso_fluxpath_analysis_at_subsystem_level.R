...
#based on combining 09.24.21_Burns2015_fluxpath_analysis_at_subsystem_level.R 
#and 12.20.21_Burns2015_Fuso_fluxpath_analysis_exchanges_only_newfilter2.R
...
#PSYCHE! Don't actually need to do this- the data comes in pre-filtered by fuso only
...
#PSYCHE! Don't actually need to do this 
...
analyzed_fluxpaths <- read_csv("01.04.22_Burns2015_Fuso_fluxes_with_metadata_longform.csv") 
...
fluxpaths_unique <- unique(analyzed_fluxpaths$fluxpath_name) #6188 fluxpaths
#how many PBIDs?
PBIDs_unique <- unique(analyzed_fluxpaths$Patient_Blind_ID) #22 PBIDs
#how many PBIDs?
...
#########NEW Step 3: collapse this dataset by subsystem; average across fluxpaths within a subsystem within a sample.
#drop unnecessary columns
analyzed_fluxpaths <- analyzed_fluxpaths %>% rename("fluxpath_description"="description", "fluxpath_subsystem"="subsystem")
...
#now, remove all rows where n (the name of the count column) is less than ...22, which is half the samples
analyzed_fluxpaths_filtered <- filter(analyzed_fluxpaths_filter_added, n >= 22)
...
write.csv(as.data.frame(fluxpath_statistics), file="01.04.22_Fusobacterium_flux_subsystems_stats_Burns2015_data.csv")
...
PBIDs_unique <- unique(analyzed_fluxpaths$Patient_Blind_ID) #22 PBIDs
#clean up
rm(fluxpaths_unique, PBIDs_unique)

#########Step 3: filtering NOT NECESSARY HERE
...
fuso_fluxpaths_only_filtered <- analyzed_fluxpaths
...
fuso_fluxpaths_collapsed$description[fuso_fluxpaths_collapsed$description== "Co2+ exchange"] <- "Exchange of nine trace elements"
...
num_fuso_pos_samples <- unique(fuso_fluxpaths_only_filtered$SampleID) #44 samples, here.CHECK.

#first, create a filtering dataframe that contains counts of each fluxpath_name incidence in the dataframe
fuso_fluxpaths_filtering_df <- fuso_fluxpaths_wide_noNANs_zerofree %>%  #create a new dataframe by filtering the old one
  mutate(count_of_nonzeros = rowSums(.[8:9]!=0)) %>% #create a new column, count_of_nonzeroes, that counts # nonzero values in tumor and normal columns
...
fuso_fluxpaths_paired_only <- select(fuso_fluxpaths_abundance_filtered, -c(...1, difference, n, fluxpath_description, abbreviation, formula, subsystem))
#convert to long form
...
write.csv(as.data.frame(fuso_fluxpath_statistics2), file="01.04.22_Fusobacterium_all_flux_stats_Burns2015_data.csv")
