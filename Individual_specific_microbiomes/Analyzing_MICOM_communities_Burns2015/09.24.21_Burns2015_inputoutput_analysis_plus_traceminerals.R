...
#same as 09.10.21_Burns2015_inputoutput_analysis.R, but with added step of collapsing trace minerals components, and also based on 09.20.21_Burns2015_inputoutput_analysis_plus_traceminerals.R
...
#########Step 2: filter to only include paired taxa (i.e. taxa which appear in both tumor and normal samples)
analyzed_metabolites_wide <- select(analyzed_metabolites_collapsed, -c(...1, sample_ID_ish)) #drop unnecessary columns
...
#now, remove all rows where n (the name of the count column) is less than 18
...
write.csv(as.data.frame(metabolite_statistics), file="09.24.21_filtered_microbial_input_output_all_results_Burns2015_data_trace_elements_combined.csv")
...
#Split up to wide.
#analyzed_metabolites_paired_wide <- spread(analyzed_metabolites_paired_only, Description, metabolite_amount)
#add a new column called diff. tumor-normal
analyzed_metabolites_paired_only$diff <- (analyzed_metabolites_paired_only$tumor - analyzed_metabolites_paired_only$normal)

#NOW we can add a new column with useful binning of differences for graphing purposes
analyzed_metabolites_paired_wide <- analyzed_metabolites_paired_only %>%
...
write.csv(as.data.frame(analyzed_metabolites_paired_wide), file="09.24.21_filtered_microbial_input_output_data_Burns2015_data_trace_elements_combined.csv")
