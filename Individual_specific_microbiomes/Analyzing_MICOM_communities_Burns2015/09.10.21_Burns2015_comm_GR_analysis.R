...
#further based off 01.25.21_MICOM_coop_tradeoff_EUstd_diet.R and 09.03.21_Hale2018_comm_GR_analysis.R
...
#this dataframe contains the longform data; see 09.09.21_Burns2015_comm_GR_combining_files_and_adding_metadata.py for generating it. 
group_GR_all <- read_csv("09.09.21_Burns2015_V30MSIrun_community_GRs_combined_plus_ranks.csv")
...
##########step 2: graphing!
#a terrible graph.
ggplot(group_GR_all, aes(x=Patient_Blind_ID, y=comm_gr, colour= Description)) +
  geom_dotplot(aes(fill= Description), binaxis = "y", stackdir= "center", dotsize = 0.4) + 
  ggtitle("Average community growth rates by Patient_Blind_ID,\nBurns2015 dataset") +
...
ggplot(group_GR_all, aes(x=Description, y=comm_gr)) +
  ...
  geom_line(aes(group= Patient_Blind_ID)) +
  ggtitle("Average community growth rates by tumor/normal status,\nBurns2015 dataset") +
  theme(plot.title = element_text(hjust=0.5)) +
  scale_y_continuous(name = "Average community growth\nrate, mmol/(gDW * h)", limits=c(0,0.2)) +
  scale_fill_brewer(palette = "Set1") +
  scale_color_brewer(palette = "Set1") +
...
#now, collapse the dataframe by Patient_Blind_ID, so there are no blanks in comm_gr_normal and comm_gr_tumor columns
merge_fun <- function(x) x[!is.na(x)]
group_GR_all_wide2 <-group_GR_all_wide %>%
  group_by(Patient_Blind_ID) %>%
  summarise_all(funs(merge_fun))
#that, weirdly, duplicated a bunch of rows. Remove these.
group_GR_all_wide2 <- group_GR_all_wide2[!duplicated(group_GR_all_wide2$Patient_Blind_ID), ]
...
#is tumor or normal GR higher?
mean(group_GR_all_wide2$comm_gr_normal)
mean(group_GR_all_wide2$comm_gr_tumor)
...
ggpaired(group_GR_all_wide2, cond1= "comm_gr_normal", cond2= "comm_gr_tumor",
  ...
  labs(title = expression("Community growth rates, Burns2015 data"), subtitle = expression("paired Wilcoxan, p=0.0407")) +
