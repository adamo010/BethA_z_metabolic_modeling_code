...
#and my metabolic models (based on the Burns microbiome data) and correlate them. SPECIFICALLY to get them into Cytoscape
...
#and 12.21.21_correlating_Burns_GEs_and_Fuso_fluxes.R
...
#this creates a matrix of p-values
#i would also like a matrix of R-values
fluxpath_correlations_all_rvals <- as.data.frame(fluxpath_correlations_all$r) #this makes a dataframe
fluxpath_correlations_all_rvals <- tibble::rownames_to_column(fluxpath_correlations_all_rvals, "fluxpath_name") #THANKS DPLYR, hero of R!
...
################Filtering and graphing##############
#step 1: get correlations into a longform dataframe with the columns Taxon, Metabolite, and P-value and R-value
...
#
fluxpath_correlations_all_rvals_long <- fluxpath_correlations_all_rvals %>%
  melt(id.var="fluxpath_name") %>%
  arrange(fluxpath_name, variable) 
fluxpath_correlations_all_rvals_long <- rename(fluxpath_correlations_all_rvals_long, c("gene_id"="variable", "R_value"="value")) #rename columns
...
fluxpath_correlations_all_long_merged <- merge(fluxpath_correlations_all_corrected_long,
                                               fluxpath_correlations_all_uncorrected_long,
                                               by=c("fluxpath_name","gene_id"))
fluxpath_correlations_all_long_merged <- merge(fluxpath_correlations_all_long_merged,
                                               fluxpath_correlations_all_rvals_long,
                                               by=c("fluxpath_name","gene_id"))
...
write.csv(as.data.frame(fluxpath_correlations_all_long_merged), file="03.24.22_Burns2015_metabolic_gene_expression_Fuso_only_fluxpath_correlations_for_cytoscape.csv")

#network was a mess, so let's filter
#only keep rows where unadjusted P<0.05

fluxpath_correlations_all_long_merged_sigonly <- filter(fluxpath_correlations_all_long_merged, p_value_raw < 0.05)
write.csv(as.data.frame(fluxpath_correlations_all_long_merged_sigonly), file="03.24.22_Burns2015_metabolic_gene_expression_Fuso_only_fluxpath_correlations_for_cytoscape_sigonly.csv")

#still too many. Only keep top 100
fluxpath_correlations_all_long_merged_top100 <- fluxpath_correlations_all_long_merged %>%
  arrange(desc(p_value_raw)) %>%
  slice(1:100)
write.csv(as.data.frame(fluxpath_correlations_all_long_merged_top100), file="03.24.22_Burns2015_metabolic_gene_expression_Fuso_only_fluxpath_correlations_for_cytoscape_top100.csv")
