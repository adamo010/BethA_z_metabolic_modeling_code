...
unique_fluxpaths <- unique(analyzed_fluxpaths$fluxpath_name)
...
num_fuso_pos_samples <- unique(fuso_fluxpaths_collapsed$Sample_ID) #44 samples, here
...
write.csv(as.data.frame(fuso_fluxpath_statistics2), file="11.17.21_Fusobacterium_exchange_fluxes_only_stats_Burns2015_data.csv")
...
write.csv(as.data.frame(fuso_fluxpaths_paired_wide2), file="11.17.21_Fusobacterium_exchange_fluxes_only_alldata_Burns2015_data.csv")
...
ggpaired(subset(fuso_fluxpaths_paired_wide2, fluxpath_name %in% c("EX_mqn8(e)")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Fusobacterium menaquinone8 exchange flux, Burns2015 data"), subtitle = expression("paired Wilcoxan, p=0.000418, q= 0.00568")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
