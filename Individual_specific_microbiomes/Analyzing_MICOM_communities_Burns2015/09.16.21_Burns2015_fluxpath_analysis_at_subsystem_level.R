...
#furtherer based on 09.10.21_Burns2015_fluxpath_analysis.R; modification here is that we're looking at subsystems, rather than just fluxpaths.
...
analyzed_fluxpaths_subset_averaged <- analyzed_fluxpaths_subset_noNANs %>%        # Specify data frame
  group_by(sorting_col, Sample_ID, Description, Patient_Blind_ID, fluxpath_subsystem) %>%    # Specify group indicator (columns to keep)
...
analyzed_fluxpaths_filtered$sorting_col2 <- paste(analyzed_fluxpaths_filtered$fluxpath_subsystem, "_", analyzed_fluxpaths_filtered$Patient_Blind_ID)
...
by_flux_stats <- lapply(split_analyzed_fluxpaths_by_flux, function(df){wilcox.test(av_fluxpath_amount~Description, data=df, exact= FALSE, paired= TRUE)})
...
write.csv(as.data.frame(top_twenty_fluxpaths), file="09.16.21_filtered_microbial_flux_subsystems_top20_sig_results_Burns2015_data.csv")
...
#Split up to wide.
analyzed_fluxpaths_paired_wide <- spread(analyzed_fluxpaths_paired_wide, Description, av_fluxpath_amount)
...
write.csv(as.data.frame(analyzed_fluxpaths_paired_wide), file="09.16.21_filtered_microbial_flux_subsystems_top20_data_Burns2015_data.csv")

#step 10: NOW GRAPH??!!!
#Fatty acid oxidation
ggpaired(subset(analyzed_fluxpaths_paired_wide, fluxpath_subsystem %in% c("Fatty acid oxidation")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Fatty acid oxidation subsystem\n average flux, Burns2015 data"), subtitle = expression("paired Wilcoxan, p=0.0145, q= 0.520")) +
...
ggpaired(subset(analyzed_fluxpaths_paired_wide, fluxpath_subsystem %in% c("Tryptophan metabolism")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Tryptophan metabolism subsystem\n average flux, Burns2015 data"), subtitle = expression("paired Wilcoxan, p=0.0304, q= 0.520")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
...
ggpaired(subset(analyzed_fluxpaths_paired_wide, fluxpath_subsystem %in% c("Vitamin B2 metabolism")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Vitamin B2 metabolism subsystem\n average flux, Burns2015 data"), subtitle = expression("paired Wilcoxan, p=0.0405, q= 0.520")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
...
ggpaired(subset(analyzed_fluxpaths_paired_wide, fluxpath_subsystem %in% c("Alanine and aspartate metabolism")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Alanine and aspartate metabolism subsystem\n average flux, Burns2015 data"), subtitle = expression("paired Wilcoxan, p=0.0479, q= 0.520")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
