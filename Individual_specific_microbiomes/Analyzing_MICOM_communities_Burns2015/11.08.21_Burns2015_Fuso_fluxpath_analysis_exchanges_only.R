...
#ah, great: our OTU_id to look for is 1096339 FOR THIS DATASET. There should only be one, but double check just in case
...
#analyzed_fluxpaths$Patient_Blind_ID <- as.factor(analyzed_fluxpaths$Patient_Blind_ID) #do this after filtering
...
#but, do need to filter out PBIDs where Fuso is only present in tumor or normal.
fuso_PBIDs_unique <- unique(fuso_fluxpaths_only$Patient_Blind_ID) #right now, 44 PBIDs, which is...how is that possible?
#fuso isn't even present in all PBIDs?
all_PBIDs_unique <- unique(analyzed_fluxpaths$Patient_Blind_ID)
#I wonder if this table is pre-abundance filtering. Gotta be, right?
#okay, time for something more crafty. 

#a post-filtered abundance table:
...
paired_fuso_PBIDs2 <- unique(paired_fuso_PBIDs$Patient_Blind_ID) 
...
#okay, the key here was (for some dumb fuck reason) to NOT set Patient_Blind_ID as factor before changing.
...
fuso_fluxpaths_trace_minerals_only <- fuso_fluxpaths_only[fuso_fluxpaths_only$fluxpath_name %in% trace_minerals, ]
...
fuso_fluxpaths_collapsed <-data.frame(fuso_fluxpaths_only) #copy the original dataframe
...
num_fuso_pos_samples <- unique(fuso_fluxpaths_collapsed$Sample_ID) #60 samples, here
...
#now, remove all rows where n (the name of the count column) is less than ... 60 is 100%, 30 is 50%, 12 is 20%
fuso_fluxpaths_abundance_filtered <- filter(fuso_fluxpaths_filter_added, n >= 30)
...
write.csv(as.data.frame(fuso_fluxpath_statistics2), file="11.08.21_Fusobacterium_exchange_fluxes_only_stats_Burns2015_data.csv")
...
write.csv(as.data.frame(fuso_fluxpaths_paired_wide2), file="11.08.21_Fusobacterium_exchange_fluxes_only_alldata_Burns2015_data.csv")

#step 10: graphing!
#Biomass
ggpaired(subset(fuso_fluxpaths_paried_wide2, fluxpath_name %in% c("EX_biomass(e)")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Fusobacterium biomass flux, Burns2015 data"), subtitle = expression("paired Wilcoxan, p=0.000471, q= 0.00568")) +
...
#Heme
ggpaired(subset(fuso_fluxpaths_paried_wide2, fluxpath_name %in% c("EX_pheme(e)")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Fusobacterium heme exchange flux, Burns2015 data"), subtitle = expression("paired Wilcoxan, p=0.000418, q= 0.00568")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
  scale_y_continuous(name = "Mean flux,\nmmol/(gDW * h)", limits=c(-0.00005,0)) +
  scale_x_discrete(labels = c('Normal','Tumor')) +
  scale_fill_brewer(palette = "Dark2") +
  scale_color_brewer(palette = "Dark2") +
  theme(legend.position = "bottom", legend.title=element_blank(), legend.text=element_text(size=11)) +
...
#Exchange of Thiamin
ggpaired(subset(fuso_fluxpaths_paried_wide2, fluxpath_name %in% c("EX_thm(e)")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Fusobacterium thiamin exchange flux,\n Burns2015 data"), subtitle = expression("paired Wilcoxan, p=0.000418, q= 0.00568")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
  scale_y_continuous(name = "Mean flux,\nmmol/(gDW * h)", limits=c(-0.00005,0.0000001)) +
  scale_x_discrete(labels = c('Normal','Tumor')) +
  scale_fill_brewer(palette = "Dark2") +
  scale_color_brewer(palette = "Dark2") +
  theme(legend.position = "bottom", legend.title=element_blank(), legend.text=element_text(size=11)) +
...
ggpaired(subset(fuso_fluxpaths_paried_wide2, fluxpath_name %in% c("Trace_elements_exchange")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Fusobacterium trace element exchange flux,\n Burns2015 data"), subtitle = expression("paired Wilcoxan, p=0.000473, q= 0.00568")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
