...
#how many unique fluxes are we including here? 
fluxpaths_averaged_unique <- unique(analyzed_fluxpaths_subset_averaged$fluxpath_subsystem)

#########Step 3: filter to only include paired taxa (i.e. taxa which appear in both tumor and normal samples)
analyzed_fluxpaths_wide <- select(analyzed_fluxpaths_subset_averaged, -c(sorting_col, Sample_ID)) #drop unnecessary columns
...
write.csv(as.data.frame(fluxpath_statistics), file="09.24.21_filtered_microbial_flux_subsystems_all_results_Burns2015_data.csv")
...
write.csv(as.data.frame(fluxpath_statistics_sig), file="09.24.21_filtered_microbial_flux_subsystems_SIG_results_Burns2015_data.csv")
...
write.csv(as.data.frame(analyzed_fluxpaths_paired_only), file="09.24.21_filtered_microbial_flux_subsystems_all_data_Burns2015_data.csv")
...
ggpaired(subset(analyzed_fluxpaths_paired_wide, fluxpath_subsystem %in% c("Vitamin B2 metabolism")), cond1= "normal", cond2= "tumor",
         fill = "condition", line.color = "Difference_direction") +
  labs(title = expression("Vitamin B2 metabolism subsystem\n average flux, Burns2015 data"), subtitle = expression("paired Wilcoxan, p=0.0239, q= 0.563")) +
  theme(plot.title = element_text(hjust=0.5), plot.subtitle= element_text(hjust=0.5), plot.margin = unit(c(1,0.1,0.1,0.1), "cm")) +
