...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 15 15:50:06 2021
...
otus = pd.read_csv("/Users/adamo010/Documents/MICOM_CRC_FBA/Model_picking/Hale_2018_model_picking/07.13.21_Hale2018_OTUs_without_models_V1_indiv_specific_comms.csv") #this is the new contaminant-free table
...
for row in unique_mambo_species['species_level_sort']:  
    ...
    del templist, species_name_key, row 
del unique_mambo_species    
...
for row in unique_mambo_species2['species2_level_sort']:  
    ...
    del templist, species2_name_key, row    
del unique_mambo_species2 
...
otus_plus_tax = otus_plus_tax.drop(columns={"Unnamed: 0", "taxonomy_orig_y", "species_matched_model_list", "genus_matched_model_list", "family_matched_model_list"})
...
#NOTE! ditto as above; don't need to do the exploding if we're using the AGORA output
#all right, here is where we're going to need to be careful with Hale2018 data. There's some weird shit going on at the species/genus level 
#that means there are multiple species and genus ids. How do we handle this?
#EXPLODE!!!
#first, convert Species column to list (instead of strings delimited by /)
#def convert_to_list(x):
	#return x.split("/")
#species_matched_models["Species"] = species_matched_models["Species"].apply(convert_to_list)
#species2_matched_models["Species"] = species2_matched_models["Species"].apply(convert_to_list)
...
#now explode!
#species_matched_models= species_matched_models.explode('Species')
#species2_matched_models= species2_matched_models.explode('Species')
#spectacular. Now, back to matching models.

#step 0: new species_level_sort column for this. 
#DON"T NEED THIS FOR MAMBO MODELS IF AGORA DONE FIRST
#species_matched_models["clean_species_level_sort"] = species_matched_models["genus_level_sort"] + "; " + species_matched_models['Species']
#species2_matched_models["clean_species_level_sort"] = species2_matched_models["genus_level_sort"] + "; " + species2_matched_models['Species']

####step 1: map species models to species-level list
species_matched_models['species_matched_model_list'] = species_matched_models['clean_species_level_sort'].map(species_models_dict)        
...
#NOTE: FOR MAMBO MODELS, THIS SHOULD BE EMPTY AS THESE OTUS WERE REMOVED IN AGORA CODE
#otus_with_low_tax_ids = otus_plus_tax[otus_plus_tax['Family'].isnull() & otus_plus_tax['Genus'].isnull() & otus_plus_tax['Species'].isnull()]   
...
otus_with_models_V1.to_csv("07.15.21_Hale2018_OTUs_with_MAMBO_models_V1_indiv_specific_comms.csv")
otus_with_models_V2.to_csv("07.15.21_Hale2018_OTUs_with_MAMBO_models_V2_indiv_specific_comms.csv")
#otus_with_low_tax_ids.to_csv("07.13.21_Hale2018_OTUs_with_low_tax_ids.csv")
otus_without_family_models.to_csv("07.15.21_Hale2018_OTUs_without_MAMBO_models_V1_indiv_specific_comms.csv") 
otus_without_family2_models.to_csv("07.15.21_Hale2018_OTUs_without_MAMBO_models_V2_indiv_specific_comms.csv") 
