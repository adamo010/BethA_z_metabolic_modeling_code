...
#update from 04.22.20: fixed the contaminants list
...
mambo = pd.read_csv("04.09.20_MAMBO_model_taxonomy_assigned.csv")       
otus = pd.read_csv("06.16.20_OTUs_without_models_V1.csv") #at least V1 and V2 are identical
...
otus_columns_list = otus.columns.tolist()

otus[otus_columns_list] = otus[otus_columns_list].replace({'\[':''}, regex=True)
otus = otus.replace({'\]':''}, regex=True)
#note that both of these work; I was running into an 'error: unterminated character set' with getting rid of [, ]
...
#great, now we have some otus (at species/genus/family level) with models assigned
...
otus_with_models_V1.to_csv("06.16.20_OTUs_with_mambo_models_V1.csv")

V2_list = [otus_with_species2_models, otus_with_genus2_models, otus_with_family2_models]
otus_with_models_V2 = pd.concat(V2_list)
otus_with_models_V2.to_csv("06.16.20_OTUs_with_mambo_models_V2.csv")
...
otus_without_models.to_csv("06.16.20_OTUs_without_mambo_models_V1.csv")
wo_models2 = [otus_without_family2_models, otus_with_low_tax_ids]
otus_without_models2 = pd.concat(wo_models2)
otus_without_models2.to_csv("06.16.20_OTUs_without_mambo_models_V2.csv")

#cool. how many extra models did we get?    THREE!!!
