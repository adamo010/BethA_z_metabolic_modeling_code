...
#and 07.29.21_combining_CarveMe_models_for_Hale2018_otus.py
...
####AGORA V1
agora_V1 = pd.read_csv("12.01.21_Muehlbauer2021_OTUs_with_models_V1_indiv_specific_comms.csv")   
sorted(agora_V1) #prints column names
agora_V1 = agora_V1.drop(columns=["Unnamed: 0", 'class_level_sort',"clean_species_level_sort",\
                                  'family_level_sort','species_level_sort', 'genus_level_sort',\
                                      'keep_or_toss','order_level_sort','phylum_level_sort','taxonomy_orig_y'])  
#goal: keep OTU_id, sample_ids, Kingdom-Genus, taxonomy_orig_x, all_agora_matched_models

#make a list of the columns which need nans replaced with blank lists
list_of_cols = ["species_matched_model_list", "genus_matched_model_list", "family_matched_model_list"]
...
agora_V1["all_agora_matched_models"] = agora_V1["family_matched_model_list"] + agora_V1["genus_matched_model_list"] + agora_V1["species_matched_model_list"]
del list_of_cols
#drop unnecessary columns.
agora_V1 = agora_V1.drop(columns=["species_matched_model_list", "genus_matched_model_list", "family_matched_model_list"])
    
####AGORA V2
agora_V2 = pd.read_csv("12.02.21_Muehlbauer2021_OTUs_with_agora_models_fg_level_picks.csv")
sorted(agora_V2)
#for V2, already have an all_models column. so, can delete other matched_model_list columns. 
agora_V2 = agora_V2.drop(columns=["Unnamed: 0", 'agora_fg2_matched_model_list','agora_fg_matched_model_list',\
                                  'agora_fgs2_matched_model_list','agora_fgs_matched_model_list','agora_gs2_matched_model_list',\
                                      'agora_gs_matched_model_list','class_level_sort','clean_species_level_sort',\
                                          'family_level_sort','fg_level_sort','fgs_level_sort','genus_level_sort',\
                                              'gs_level_sort', 'order_level_sort', 'phylum_level_sort','species_level_sort',\
                                                  'species_matched_model_list'])
...
####AGORA V3
agora_V3 = pd.read_csv("12.02.21_Muehlbauer2021_OTUs_with_agora_models_single_tax_level_picks.csv")
sorted(agora_V3)
#for V3, already have an all_models column. so, can delete other matched_model_list columns. 
agora_V3 = agora_V3.drop(columns=['Unnamed: 0','agora_family_matched_model_list', 'agora_genus2_matched_model_list',\
                                  'agora_genus_matched_model_list','agora_species_matched_model_list',\
                                      'class_level_sort','clean_species_level_sort','family_level_sort','genus_level_sort',\
                                          'order_level_sort','phylum_level_sort', 'species_level_sort', 'species_matched_model_list'])
...
######AGORA V4
agora_V4= pd.read_csv("12.02.21_Muehlbauer2021_with_agora_models_single_tax_level_hand_picks.csv")
sorted(agora_V4)
#for V3, already have an all_models column. so, can delete other matched_model_list columns. 
agora_V4 = agora_V4.drop(columns=['Unnamed: 0','agora_family_matched_model_list', 'agora_genus2_matched_model_list',\
                                  'agora_genus_matched_model_list','agora_species_matched_model_list', "all_agora_matched_models",\
                                      'all_mambo_matched_models', 'class_level_sort','clean_species_level_sort','family_level_sort',\
                                          'genus_level_sort','mambo_family_matched_model_list','mambo_genus_extracted_matched_model_list',\
                                              ...
                                              'species_matched_model_list'])
#need to also rename family column
agora_V4= agora_V4.drop(columns="family")
agora_V4= agora_V4.rename(columns={"hand_assigned_family":"family"})
#AAAAnd rename the model matched column
agora_V4 = agora_V4.rename(columns= {"agora_family_matched_model_list_hand_assigned": "all_agora_matched_models"})
#and, do still need to convert all_agora_models column to list
...
#clean up
del agora_V1, agora_V2, agora_V3, agora_V4, all_agora_merged, all_agora_merged_slimmed

####MAMBO V1
mambo_V1 = pd.read_csv("12.02.21_Muehlbauer2021_OTUs_with_MAMBO_models_V1_indiv_specific_comms.csv")
sorted(mambo_V1) #prints column names
mambo_V1 = mambo_V1.drop(columns=["Unnamed: 0", 'class_level_sort',"clean_species_level_sort",\
                                  'family_level_sort','species_level_sort', 'genus_level_sort',\
                                      'order_level_sort','phylum_level_sort'])  
#goal: keep OTU_id, sample_ids, Kingdom-Genus, taxonomy_orig_x, all_mambo_matched_models
...
#create a merged column with all AGORA models   
mambo_V1["all_mambo_matched_models"] = mambo_V1["family_matched_model_list"] + mambo_V1["genus_matched_model_list"] + mambo_V1["species_matched_model_list"]
del list_of_cols
#drop unnecessary columns.
mambo_V1 = mambo_V1.drop(columns=["species_matched_model_list", "genus_matched_model_list", "family_matched_model_list"])

####Step 3: All CarveMe models
carveme = pd.read_csv("12.03.21_Muehlbauer2021_OTUs_with_carveme_models.csv")
sorted(carveme)
carveme= carveme.drop(columns=["Unnamed: 0", 'agora_family_matched_model_list_hand_assigned','carveme_family_matched_model_list',\
                               'carveme_genus_matched_model_list','carveme_species_matched_model_list','class_level_sort',\
                                   'clean_species_level_sort','family_level_sort','genus_level_sort','hand_assigned_family',\
                                       'mambo_species_extracted_matched_model_list','order_level_sort','phylum_level_sort',
                                       'species_level_sort'])
carveme['all_carveme_matched_models'] = carveme.all_carveme_matched_models.apply(lambda x: literal_eval(str(x)))

####step 4: import and match OTU_id keys. NOT NEEDED HERE.
...
matched_model_id_list = all_agora_merged_condensed["all_agora_matched_models_x"].to_list() #convert model ID column to list
all_model_files = [] 
...
all_agora_merged_condensed['all_agora_model_file_names'] = all_model_files
...
all_agora_merged_condensed.to_pickle('Muehlbauer2021_AGORA_models_that_need_OTU_builds.pickle')
#testo = pd.read_pickle('Muehlbauer2021_AGORA_models_that_need_OTU_builds.pickle')
mambo_V1.to_pickle('Muehlbauer2021_MAMBO_models_that_need_OTU_builds.pickle')
sorted(mambo_V1)
carveme.to_pickle("Muehlbauer2021_CarveMe_models_that_need_otu_builds.pickle")

#############testing MSI run
agora_model_file_dict = dict(zip(all_agora_merged_condensed.OTU_ID,all_agora_merged_condensed.all_agora_model_file_names.apply(lambda x: literal_eval(str(x)))))    
os.chdir("/panfs/roc/groups/7/blekhman/adamo010/Combining_Niccolai_models/AGORA_models/")
for key, value in agora_model_file_dict.items():
    filename = str(key)+"_model.xml"
    otu_model = micom.util.join_models(value, str(key))
    os.chdir("/panfs/roc/groups/7/blekhman/adamo010/Combining_Niccolai_models/OTU_models")
...
################subsetting files 
#there's some extra crap in here that I haven't deleted because, you never know. 
#os.chdir("/Users/adamo010/Documents/MICOM_CRC_FBA/Model_building_and_combining/Muehlbauer2021_model_building_and_combining")
#all_agora_merged_condensed.to_csv("all_agora_merged_condensed.csv")

#have 662 rows; divided into 4 pickle files, that is 165.5; so 166x2 and 165x2
#however, have already run a bunch of these before memory timed out; they can be found here.
#otus_with_models = pd.read_csv("OTUs_with_models_as_of_10.21.21.csv", header=None)
#otus_with_models_list = otus_with_models[0].tolist()

#remove all rows in agora model list that correspond to OTU names in this list.
#agora_subset = all_agora_merged_condensed[~all_agora_merged_condensed.OTU_ID.isin(otus_with_models_list)]
#all right, now we have 236 OTUs. Can probaby break this into two subsets and get away with it.
agora_subset1 = all_agora_merged_condensed.iloc[:118,:]
agora_subset2 = all_agora_merged_condensed.iloc[118:,:]
#save pickle files
agora_subset1.to_pickle('Muehlbauer2021_AGORA_models_that_need_OTU_builds_subset1.pickle')
agora_subset2.to_pickle('Muehlbauer2021_AGORA_models_that_need_OTU_builds_subset2.pickle')
