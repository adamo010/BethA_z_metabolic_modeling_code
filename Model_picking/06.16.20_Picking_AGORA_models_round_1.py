...
#IMPORTANT: this does NOT cover AGORA model picking where manual taxonomy assignment was required. That is covered adequately in 05.01.20
#update from 04.28: found some more models with issues.
...
otus = pd.read_csv("06.16.20_otu_table_abundance_filtered_0.1_edited_taxonomies.csv") #this is the new contaminant-free table
...
otus["phylum_level_sort"] = np.where(len(otus["phylum"]) > 1, (otus["kingdom"] + ";" + otus['phylum']), "")
otus["class_level_sort"] = np.where(len(otus["class"]) > 1, (otus["phylum_level_sort"] + ";" + otus['class']), "")
otus["order_level_sort"] = np.where(len(otus["order"]) > 1, (otus["class_level_sort"] + ";" + otus['order']), "")
otus["family_level_sort"] = np.where(len(otus["family"]) > 1, (otus["order_level_sort"] + ";" + otus['family']), "")
otus["genus_level_sort"] = np.where(len(otus["genus"]) > 1, (otus["family_level_sort"] + ";" + otus['genus']), "")
otus["species_level_sort"] = np.where(len(otus["species"]) > 1, (otus["genus_level_sort"] + ";" + otus['species']), "")

otus["phylum_level_sort"] = otus["phylum_level_sort"].str.strip(' ;')
otus["class_level_sort"] = otus["class_level_sort"].str.strip(' ;')
otus["order_level_sort"] = otus["order_level_sort"].str.strip(' ;')
otus["family_level_sort"] = otus["family_level_sort"].str.strip(' ;')
otus["genus_level_sort"] = otus["genus_level_sort"].str.strip(' ;')
otus["species_level_sort"] = otus["species_level_sort"].str.strip(' ;')
...
def remove_blanks(otu_table, colname):
    for elem in otu_table[str(colname)]:
        if len(elem) <=4:
            dummy = elem
    otu_table[str(colname)].replace({dummy: None}, inplace = True)
    del dummy
    del elem
    return
...
remove_blanks(otus, "phylum")
remove_blanks(otus, "class")
remove_blanks(otus, "order")
remove_blanks(otus, "family")
remove_blanks(otus, "genus")
remove_blanks(otus, "species")    

otus_to_species_level = otus[otus['species'].notnull()]   #hey, look at that

#now for the harder part
otus_to_genus_level = otus[otus['genus'].notnull() & otus['species'].isnull()]   #you may award me the genuis medal now
otus_to_family_level = otus[otus['family'].notnull() & otus['genus'].isnull() & otus['species'].isnull()]   
...
otus_with_models_V1.to_csv("06.16.20_OTUs_with_models_V1_taxonomy_fixed.csv")
otus_with_models_V2.to_csv("06.16.20_OTUs_with_models_V2_taxonomy_fixed.csv")
otus_with_low_tax_ids.to_csv("06.16.20_OTUs_with_low_tax_ids_taxonomy_fixed.csv")
otus_without_family_models.to_csv("06.16.20_OTUs_without_models_V1.csv") #these should be empty anyway
otus_without_family2_models.to_csv("06.16.20_OTUs_without_models_V2.csv") #these should be empty anyway
