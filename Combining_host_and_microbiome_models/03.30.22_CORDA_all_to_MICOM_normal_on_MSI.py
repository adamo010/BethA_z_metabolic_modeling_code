...
###############STEP 0: file lists and names################ #we are ONLY running normal microbiome here.
def main():
    script = sys.argv[0]
    microbiome_model = sys.argv[1] #these files are ONLY normal human files. see Burns2015_normal_pickle_file_list.txt
    ...
    metadata_subset= all_metadata[all_metadata["microbiome_model_file"]==microbiome_model_sample_prename]
    ...
    spentmed_file_names = [x.replace('_hostmodel_EUstd_spent_medium.csv', '') for x in spentmed_file_names] 
    ...
    for key, value in spentmed_dict.items():
        ...
        os.chdir("/panfs/roc/groups/7/blekhman/adamo010/Merging_Burns_MICOM_and_CORDA/MICOM_spent_med_output_round2/")
        host_plus_microbiome_name = "PBID_"+str(relevant_PBID)+"_"+str(microbiome_model_id)+"_normal_microbiome_"+"to_"+str(host_real_sampleid)+\
        ...
        rates["microbiome_model_description"] = "normal" #CHANGE WITH EACH script
