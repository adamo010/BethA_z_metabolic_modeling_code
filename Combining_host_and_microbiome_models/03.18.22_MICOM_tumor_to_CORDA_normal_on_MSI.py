...
#we are ONLY running tumor microbiome and normal human samples here today.
def main():
    script = sys.argv[0]
    ...
    microbiome_id_by_PBID_dict = dict(zip(metadata_subset.Patient_Blind_ID, metadata_subset.microbiome_sampleid))
    for key, value in host_model_dict.items():
        ...
        microbiome_plus_host_name = str("PBID_"+str(host_PBID)+"_normal_host_"+str(human_model_id)+"_tumor_microbiome_"\
