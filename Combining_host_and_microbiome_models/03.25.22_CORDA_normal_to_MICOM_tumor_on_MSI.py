...
###############STEP 0: file lists and names################ #we are ONLY running normal microbiome and tumor human samples here today. 03.25.22_CORDA_normal_to_MICOM_tumor_on_MSI.py
def main():
    script = sys.argv[0]
    microbiome_model = sys.argv[1] #these files are ONLY tumor human files. see Burns2015_tumor_pickle_file_list.txt
    ...
    metadata_subset= all_metadata[all_metadata["Pair_type"]=="normal_tumor"] #CHANGE WITH EACH script
    ...
    for file in os.listdir():
    	if str("_normal_hostmodel_EUstd_spent_medium.csv") in str(file): #CHANGE ME when using tumor/normal host
    ...
    spentmed_file_names = [x.replace('_normal_hostmodel_EUstd_spent_medium.csv', '') for x in spentmed_file_names] #CHANGE ME when using tumor/normal host
    ...
    for key, value in microbiome_model_dict.items():
        ...
        host_plus_microbiome_name = str("PBID_"+str(microbiome_PBID)+"_tumor_microbiome_"+str(microbiome_model_id)+"_normal_host_"\
        ...
        fluxes["Patient_Blind_ID"] = microbiome_PBID
        fluxes["microbiome_model_id"] = microbiome_model_id
        fluxes["microbiome_model_description"] = "tumor" #CHANGE WITH EACH script
        fluxes["host_model_id"] = spentmed_sampleid
        ...
        microbiome_post_coop_med = minimal_medium(microbiome_model, microbiome_sample_sol.growth_rate, exports=True)
        if microbiome_post_coop_med is None: 
            output = "This sample has no post coop med" 
            with open(host_plus_microbiome_name+"_EUstd_post_coop_growth_med.csv", "w") as text_file: 
                text_file.write(output) 
        else:
            microbiome_post_growth_med = microbiome_post_coop_med.to_frame() #convert series to dataframe
            microbiome_post_growth_med.rename(columns={0: "metab_amount"}, inplace=True) 
            microbiome_post_growth_med["Patient_Blind_ID"] = microbiome_PBID
            microbiome_post_growth_med["microbiome_model_id"] = microbiome_model_id
            microbiome_post_growth_med["microbiome_model_description"] = "tumor" #CHANGE WITH EACH script
            microbiome_post_growth_med["host_model_id"] = spentmed_sampleid
            microbiome_post_growth_med["host_model_description"] = "normal" #CHANGE WITH EACH script
            microbiome_post_growth_med.to_csv(host_plus_microbiome_name+"_EUstd_post_coop_growth_med.csv")
