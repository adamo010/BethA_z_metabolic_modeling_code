...
#we are ONLY running tumor microbiome and tumor human samples here today.
def main():
    script = sys.argv[0]
    ...
    for file in os.listdir():
    	if str("_tumor_microbiome_TD0.2_EUstd_spent_medium.csv") in str(file): #CHANGE ME when using tumor/normal microbiome
    ...
    spentmed_file_names = [x.replace('_tumor_microbiome_TD0.2_EUstd_spent_medium.csv', '') for x in spentmed_file_names] #CHANGE ME when using tumor/normal microbiome
    ...
    for key, value in host_model_dict.items():
        ...
        microbiome_plus_host_name = str("PBID_"+str(host_PBID)+"_tumor_host_"+str(human_model_id)+"_tumor_microbiome_"\
        ...
        human_post_growth_med["host_model_description"] = "tumor" #CHANGE WITH EACH script
        human_post_growth_med["microbiome_model_id"] = spentmed_sampleid
        human_post_growth_med["microbiome_model_description"] = "tumor" #CHANGE WITH EACH script
